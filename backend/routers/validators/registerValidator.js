...
module.exports = [
    ...
    check('role')
        ...
        .isIn(['CUSTOMER', 'ADMIN']).withMessage('Vai trò người dùng không hợp lệ.'),

    check('addresses')
        .optional()
        .isArray().withMessage('Địa chỉ phải là một danh sách.')
        .custom((addresses) => {
            if (!addresses.every(address => 
                address.full_name && typeof address.full_name === 'string' &&
                address.phone && typeof address.phone === 'string' &&
    ...
    check('social_auth')
        .optional()
        .isObject().withMessage('Xác thực xã hội phải là một đối tượng.')
        .custom((auth) => {
            if (auth.provider && !['google', 'facebook'].includes(auth.provider)) {
    ...
    check('points')
        .optional()
        .isInt({ min: 0 }).withMessage('Điểm thưởng phải là một số nguyên không âm.'),
