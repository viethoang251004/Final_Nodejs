...
module.exports = [
    ...
    check('category')
        .exists().withMessage('Vui lòng cung cấp danh mục sản phẩm.')
        .notEmpty().withMessage('Danh mục sản phẩm không được để trống.')
        .trim(),

    check('tags')
        .optional()
        .isArray().withMessage('Tags phải là một danh sách.')
        .custom(tags => {
    ...
    check('variants')
        .optional()
        .isArray().withMessage('Các biến thể phải là một danh sách.')
        .custom(variants => {
            if (!variants.every(variant =>
                variant.size && typeof variant.size === 'string' &&
                variant.color && typeof variant.color === 'string' &&
                !isNaN(variant.stock) && parseInt(variant.stock) >= 0
            )) {
    ...
    check('rating')
        .optional()
        .isFloat({ min: 0, max: 5 }).withMessage('Xếp hạng phải từ 0 đến 5.'),

    check('reviews_count')
        .optional()
        .isInt({ min: 0 }).withMessage('Số lượng đánh giá phải là số nguyên không âm.'),

    check('created_at')
        .optional()
        .isISO8601().withMessage('Ngày tạo không hợp lệ.'),

    check('updated_at')
        .optional()
        .isISO8601().withMessage('Ngày cập nhật không hợp lệ.')
];
